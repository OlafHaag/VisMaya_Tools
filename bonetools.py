...
class AppendL(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        loop_num = 0
        # loop to make sure all bones end in .L, rather than .L.001 (such bones would now end in .L.001.L)
...
class AppendR(bpy.types.Operator):
    ...
    def execute(self, context):
        loop_count = 2  # number of times to loop
        loop_num = 0
        while loop_num < loop_count:  # loop to make sure all bones end in .R, rather than .R.001 (such bones would now end in .R.001.R)
            for e in bpy.context.selected_editable_bones:
...
class AutoNameSuffix(bpy.types.Operator):
    """Add a .L, .R or .M to selected bone names based on bones local X location.
    Set Middle first to prevent inaccurate naming of middle bones.
    """
...
class SelectNonSuffix(bpy.types.Operator):
    ...
    def execute(self, context):
        bpy.ops.object.select_pattern(pattern="SweetJesusThisIsOneUnlikelyBoneName!oasdbngsdhdfnLJKSDNFH",
