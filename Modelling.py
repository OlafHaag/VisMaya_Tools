...
class Extrude_Normal(bpy.types.Operator):
    ...
    location = FloatVectorProperty(name="Vector", subtype='TRANSLATION', default=(0.0, 0.0, 0.0,), min=-100.0,
                                   max=100.0, soft_min=-10, soft_max=10, step=0.1, size=3)
    
    orientation = EnumProperty(items=(
        ('one', 'Global', 'GLOBAL', 1), ('two', 'Local', 'LOCAL', 2), ('three', 'Normal', 'NORMAL', 3),
        ('four', 'Gimbal', 'GIMBAL', 4), ('five', 'View', 'VIEW', 5)), default='three', name="")
    
    propo_edit = EnumProperty(items=(
        ('one', 'Disable', 'DISABLED', 'PROP_OFF', 1), ('two', 'Enable', 'ENABLED', 'PROP_ON', 2),
        ('three', 'Projected (2D)', 'PROJECTED', 'PROP_ON', 3), ('four', 'Connected', 'CONNECTED', 'PROP_CON', 4)),
        default='one', name="")
    
    propo_edit_fall = EnumProperty(items=(
        ('one', 'Smooth', 'SMOOTH', 'SMOOTHCURVE', 1), ('two', 'Sphere', 'SPHERE', 'SPHERECURVE', 2),
        ('three', 'Root', 'ROOT', 'ROOTCURVE', 3), ('four', 'Sharp', 'SHARP', 'SHARPCURVE', 4),
        ('five', 'Linear', 'LINEAR', 'LINCURVE', 5), ('six', 'Constant', 'CONSTANT', 'NOCURVE', 6),
        ('seven', 'Random', 'RANDOM', 'RNDCURVE', 7)), default='one', name="")
    ...
    def normal(self, con_x, con_y, con_z, orien, con, fall, propsize, texture, release):
        bpy.ops.mesh.extrude_region_move(MESH_OT_extrude_region={"mirror": False},
                                         TRANSFORM_OT_translate={"value": self.location,
                                                                 "constraint_axis": (con_x, con_y, con_z),
                                                                 "constraint_orientation": orien, "mirror": False,
                                                                 "proportional": con, "proportional_edit_falloff": fall,
                                                                 "proportional_size": propsize, "snap": False,
                                                                 "snap_target": 'CLOSEST', "snap_point": (0, 0, 0),
                                                                 "snap_align": False, "snap_normal": (0, 0, 0),
                                                                 "texture_space": texture, "release_confirm": release})
                        ...
                        if v.select:
                            if v.normal.x == 1.0 or v.normal.x == -1.0:
                                ...
                                bpy.ops.transform.translate(
                                    value=(delta * v.normal.x / 50, delta * v.normal.y / 50, delta * v.normal.z / 50),
...
class Extrude_indiv_vertices(bpy.types.Operator):
    ...
    orientation = EnumProperty(items=(
        ('one', 'Global', 'GLOBAL', 1), ('two', 'Local', 'LOCAL', 2), ('three', 'Normal', 'NORMAL', 3),
        ('four', 'Gimbal', 'GIMBAL', 4), ('five', 'View', 'VIEW', 5)), default='one', name="")
    ...
    def normal(self, context, con_x, con_y, con_z, orien, con, fall, propsize, texture, release):
        ...
        bpy.ops.mesh.extrude_vertices_move(MESH_OT_extrude_verts_indiv={"mirror": False},
                                           TRANSFORM_OT_translate={"value": self.location,
                                                                   ...
                                                                   "constraint_orientation": orien, "mirror": False,
                                                                   "proportional": con,
                                                                   "proportional_edit_falloff": fall,
                                                                   "proportional_size": propsize, "snap": False,
                                                                   "snap_target": 'CLOSEST', "snap_point": (0, 0, 0),
                                                                   "snap_align": False, "snap_normal": (0, 0, 0),
                                                                   "texture_space": texture,
...
class Extrude_individual_faces(bpy.types.Operator):
    ...
    propo_edit = EnumProperty(items=(
        ('one', 'DISABLED', 'Disable', 'PROP_OFF', 1), ('two', 'ENABLED', 'Enable', 'PROP_ON', 2),
        ('three', 'PROJECTED', 'Projected (2D)', 'PROP_ON', 3), ('four', 'CONNECTED', 'Connected', 'PROP_CON', 4)),
        default='one', name="")
    propo_edit_fall = EnumProperty(items=(
        ('one', 'SMOOTH', 'Smooth', 'SMOOTHCURVE', 1), ('two', 'SPHERE', 'Sphere', 'SPHERECURVE', 2),
        ('three', 'ROOT', 'Root', 'ROOTCURVE', 3), ('four', 'SHARP', 'Sharp', 'SHARPCURVE', 4),
    ...
    def extrude(self, con, fall, release):
        ...
        bpy.ops.mesh.extrude_faces_move(MESH_OT_extrude_faces_indiv={"mirror": False},
                                        TRANSFORM_OT_shrink_fatten={"value": self.offset, "mirror": False,
                                                                    "proportional": con,
                                                                    "proportional_edit_falloff": fall,
                ...
                if totface >= 1:
                    for v in bm.faces:
                        ...
                        if v.select:
                            ...
                            delta1 = delta1 + delta
                            deltay1 = deltay1 + deltay
                            bpy.ops.transform.shrink_fatten(value=delta * 0.02, mirror=False, proportional='DISABLED',
                                                            proportional_edit_falloff='SMOOTH', proportional_size=1.0,
                                                            snap=False, snap_target='CLOSEST',
                                                            snap_point=(0.0, 0.0, 0.0), snap_align=False,
...
class Duplicate(bpy.types.Operator):
    ...
    def normal(self, context, link, con_x, con_y, con_z, orien, con, fall, propsize, texture, release):
        if context.active_object.mode == 'EDIT':
            bpy.ops.mesh.duplicate_move(MESH_OT_duplicate={"mode": self.mode},
                                        TRANSFORM_OT_translate={"value": self.location,
                                                                "constraint_axis": (con_x, con_y, con_z),
                                                                "constraint_orientation": orien, "mirror": False,
                                                                "proportional": con, "proportional_edit_falloff": fall,
            ...
            bpy.ops.object.duplicate_move(OBJECT_OT_duplicate={"linked": link, "mode": 'TRANSLATION'},
                                          TRANSFORM_OT_translate={"value": self.location,
                                                                  "constraint_axis": (con_x, con_y, con_z),
                                                                  "constraint_orientation": orien, "mirror": False,
                                                                  "proportional": con,
